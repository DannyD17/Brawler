...
namespace Battlehub.MeshDeformer2
{
    public class MeshDeformerMenu
    {
        ...
        public static void Subdivide()
        {
            ...
            if (deformer != null && deformer.Original != null)
            {
                ...
                for (int i = 0; i < deformer.Scaffolds.Length; ++i)
                {
                    ...
                    if (filter != null)
                    {
                        //Undo.RecordObject(scaffold.Obj.gameObject, "Battlehub.MeshDeformer Subdivide");
                        //Undo.RecordObject(filter, "Battlehub.MeshDeformer Subdivide");

                        Mesh colliderMesh = null;
                        if (collider != null)
                        {
                            //Undo.RecordObject(collider, "Battlehub.MeshDeformer Subdivide");
                            collider.sharedMesh = UnityEngine.Object.Instantiate(deformer.ColliderOriginal);
                            collider.sharedMesh.name = deformer.ColliderOriginal.name + " Deformed";
                            colliderMesh = collider.sharedMesh;
                        }

                        filter.sharedMesh = UnityEngine.Object.Instantiate(deformer.Original);
        ...
        public static void Append()
        {
            ...
            Undo.RegisterCreatedObjectUndo(deformer.Extend(), "Battlehub.MeshDeformer.Append");
            EditorUtility.SetDirty(deformer);
            Selection.activeGameObject = deformer.GetComponentsInChildren<ControlPoint>(true).Last().gameObject;
        }

        [MenuItem("Tools/Mesh Deformer/Prepend _%2", validate = true)]
        ...
        [MenuItem("Tools/Mesh Deformer/Prepend _%2")]
        public static void Prepend()
        {
            ...
            Undo.RegisterCreatedObjectUndo(deformer.Extend(true), "Battlehub.MeshDeformer.Prepend");
            EditorUtility.SetDirty(deformer);
            Selection.activeGameObject = deformer.GetComponentsInChildren<ControlPoint>(true).First().gameObject;
        }

        [MenuItem("Tools/Mesh Deformer/Straighten _%3", validate = true)]
        ...
        [MenuItem("Tools/Mesh Deformer/Straighten _%3")]
